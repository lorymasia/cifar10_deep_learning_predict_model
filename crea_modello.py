...
from tensorflow.keras.applications import NASNetMobile, MobileNetV2, EfficientNetB1 #https://keras.io/api/applications/
...
if __name__ == "__main__":
    ...
    (ds_train, ds_val, ds_test), ds_info = tfds.load(
        'cifar10',
        split=[
            'train[:80%]',   # 40.000 esempi
            'test[:30%]',    # 3.000 esempi (val)
            'test[30%:35%]'  # 1.500 esempi (test per inferenza)
        ],
        shuffle_files=True,
        as_supervised=True,
        with_info=True
